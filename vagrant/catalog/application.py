...
def showCategoryItems(category_name):
    ...
    return render_template('category.html', categories=categories, items=items, category_name=category_name)

@app.route('/catalog/<string:category_name>/<string:item_title>')
def showItem(category_name, item_title):
    item = session.query(Item).filter_by(title=item_title).join(Category).filter_by(name=category_name).one();
    creator = 0
...
def addItem():
    ...
    if request.method == 'POST':
      ...
      if session.query(Item).filter_by(title=title).count() == 0:
        ...
        newItem = Item(user_id = login_session['user_id'], category=category, title=title
              ,description=description)
        session.add(newItem)
        session.commit()
        flash('New Item %s Succesfully Created' % (newItem.title))
      else:
        flash('Item %s Not Created because already exist other Item with the same Title' % (title))
...
def editItem(item_title):
    if 'username' not in login_session:
      login_session['lasturl'] = url_for('editItem', item_title=item_title)
      return redirect('/login')
    edititem = session.query(Item).filter_by(title=item_title).one()
    if edititem.user_id != login_session['user_id']:
      return "<script>function myFunction() {alert('You are not authorized to edite this Item. Please create your own Item in order to edit.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
      title = bleach.clean(request.form['title'])
      category = session.query(Category).filter_by(name=request.form['category']).one()
      description = bleach.clean(request.form['description'])
      if title == item_title:
        ...
        if session.query(Item).filter_by(title=title).count() == 0:
          ...
          flash('Item Can\'t  be Edited because already exist another item with the same Title')
      return redirect(url_for('showCategoryItems', category_name=category.name))
    else:
      categories = session.query(Category).order_by(asc(Category.name)).all()
...
def deleteItem(item_title):
    if 'username' not in login_session:
      login_session['lasturl'] = url_for('deleteItem', item_title=item_title)
      return redirect('/login')
    ...
    if request.method == 'POST':
      deleteitem = session.query(Item).filter_by(title=request.form['item_title']).one()
